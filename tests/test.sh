...
export TRUSTCTL_LATEST_RUBRIST_VERSION=1.2.3
export TRUSTCTL_LATEST_IRONSIDE_VERSION=1.2.3

fixture_bundle rubrist 1.2.3
fixture_bundle ironside 1.2.3
fixture_bundle rubrist 1.2.4
fixture_bundle ironside 1.2.4
fixture_bundle rubrist 1.2.5
fixture_bundle rubrist 2.0.0
...
TRUSTCTL_LATEST_RUBRIST_VERSION='' \
  TRUSTCTL_TEST_CURL_RESPONSE='{"tag_name":"v2.0.0","draft":false,"prerelease":false}' \
  "${repo_dir}/bin/trustctl" install rubrist --root "$discovery_root" --no-start >"${work_dir}/discovery.out"
assert_contains "$discovery_root/rubrist/.env" "RUBRIST_VERSION=2.0.0"
...
assert_contains "${work_dir}/install.out" "rubrist configuration is valid"
assert_contains "${work_dir}/install.out" "ironside configuration is valid"
assert_contains "$install_root/rubrist/.env" "RUBRIST_VERSION=1.2.3"
assert_contains "$install_root/rubrist/.env" "RUBRIST_AUTH_SECRET="
assert_contains "$install_root/rubrist/.env" "RUBRIST_POSTGRES_PASSWORD="
assert_contains "$install_root/rubrist/.env" "RUBRIST_PUBLIC_URL=http://localhost:8081"
if grep -q '^COEVAL_' "$install_root/rubrist/.env"; then fail "old environment prefix was generated"; fi
assert_contains "$install_root/ironside/.env" "IRONSIDE_VERSION=1.2.3"
assert_contains "$install_root/.trustctl/state.env" "RUBRIST_INSTALLED=1"
...
[[ "$(sha256_file "$install_root/rubrist/compose.yaml")" == "$(awk 'NR == 1 { print $1 }' "$install_root/.trustctl/managed/rubrist-compose.sha256")" ]] \
  || fail "managed Rubrist checksum was not recorded"
...
assert_contains "${work_dir}/doctor.out" "rubrist: managed bundle"
assert_contains "${work_dir}/doctor.out" "ironside: managed bundle"
pass "doctor validates both managed products"

export TRUSTCTL_LATEST_RUBRIST_VERSION=1.2.4
export TRUSTCTL_LATEST_IRONSIDE_VERSION=1.2.3
"$install_root/trustctl" update --check --root "$install_root" >"${work_dir}/check.out"
assert_contains "${work_dir}/check.out" "Rubrist update available: 1.2.3 -> 1.2.4"
assert_contains "${work_dir}/check.out" "Ironside 1.2.3 is current."
pass "update check compares independent exact versions"

printf '\n# operator override\n' >> "$install_root/rubrist/compose.override.yaml"
before_compose="$(sha256_file "$install_root/rubrist/compose.yaml")"
before_env="$(sha256_file "$install_root/rubrist/.env")"
before_override="$(sha256_file "$install_root/rubrist/compose.override.yaml")"
"$install_root/trustctl" update rubrist --root "$install_root" --version 1.2.4 --preflight-only >"${work_dir}/preflight.out"
assert_contains "${work_dir}/preflight.out" "active Compose, version, and containers are unchanged"
[[ "$before_compose" == "$(sha256_file "$install_root/rubrist/compose.yaml")" ]] || fail "preflight changed active Compose"
[[ "$before_env" == "$(sha256_file "$install_root/rubrist/.env")" ]] || fail "preflight changed active environment"
[[ "$before_override" == "$(sha256_file "$install_root/rubrist/compose.override.yaml")" ]] || fail "preflight changed operator override"
assert_contains "$(find "$install_root/.trustctl/history/rubrist" -name update.env -print -quit)" "OUTCOME=preflighted"
assert_not_exists "$install_root/.trustctl/update.lock"
pass "preflight pulls and records without changing active state"

"$install_root/trustctl" update rubrist --root "$install_root" --version 1.2.4 --backup-confirmed >"${work_dir}/update.out"
assert_contains "${work_dir}/update.out" "rubrist updated to 1.2.4"
assert_contains "$install_root/rubrist/.env" "RUBRIST_VERSION=1.2.4"
assert_contains "$install_root/rubrist/compose.yaml" "fixture rubrist 1.2.4"
[[ "$before_override" == "$(sha256_file "$install_root/rubrist/compose.override.yaml")" ]] || fail "update changed operator override"
grep -l '^OUTCOME=healthy$' "$install_root"/.trustctl/history/rubrist/*/update.env >/dev/null \
...
expect_failure "downgrades are rejected" \
  "$install_root/trustctl" update rubrist --root "$install_root" --version 1.2.3 --backup-confirmed
...
expect_failure "non-interactive update requires backup assertion" \
  "$install_root/trustctl" update rubrist --root "$install_root" --version 1.2.5
...
expect_failure "public URLs with unsupported paths are rejected" \
  "${repo_dir}/bin/trustctl" install rubrist --root "${work_dir}/bad-url" \
    --rubrist-version 1.2.3 --rubrist-url https://example.test/rubrist --no-start
assert_contains "${work_dir}/failure.out" "must be an origin"

bad_assets="${work_dir}/bad-assets"
mkdir -p "$bad_assets/rubrist/9.9.9"
cp "${work_dir}/assets/rubrist/1.2.3/compose.yaml" "$bad_assets/rubrist/9.9.9/compose.yaml"
printf '%064d  compose.yaml\n' 0 > "$bad_assets/rubrist/9.9.9/compose.yaml.sha256"
expect_failure "release bundle checksum mismatch fails before install state is written" \
  env TRUSTCTL_ASSET_ROOT="$bad_assets" "${repo_dir}/bin/trustctl" install rubrist \
    --root "${work_dir}/bad-install" --rubrist-version 9.9.9 --no-start
...
assert_contains "${work_dir}/bootstrap.out" "trustctl 0.2.0"
pass "bootstrap can execute a locally inspected CLI"

expect_failure "old Coeval releases cannot be installed with Rubrist settings" \
  "${repo_dir}/bin/trustctl" install rubrist --root "${work_dir}/old-release" --rubrist-version 0.2.0 --no-start
assert_contains "${work_dir}/failure.out" "Rubrist requires version 0.3.0 or newer"
assert_not_exists "${work_dir}/old-release"

legacy_root="${work_dir}/legacy"
mkdir -p "$legacy_root/.trustctl"
printf 'COEVAL_INSTALLED=1\nIRONSIDE_INSTALLED=1\n' > "$legacy_root/.trustctl/state.env"
legacy_before="$(sha256_file "$legacy_root/.trustctl/state.env")"
expect_failure "old installation state is rejected without mutation" \
  "${repo_dir}/bin/trustctl" update rubrist --root "$legacy_root" --version 0.3.0 --backup-confirmed
assert_contains "${work_dir}/failure.out" "install Rubrist in a fresh root"
[[ "$legacy_before" == "$(sha256_file "$legacy_root/.trustctl/state.env")" ]] || fail "old state was changed"
